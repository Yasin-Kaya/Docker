...
Bookstore Web API Application aims to create a bookstore web service using Docker to give you the understanding to dockerization of an application. The application code is to be deployed as a RESTful web service with Flask using Dockerfile and Docker Compose on AWS Elastic Compute Cloud (EC2) Instance using Terraform.

## Problem Statement

![Project_01](Docker1288X600.jpg) 
...
|----bookstore-api.py   # Given (Python Flask Web API)
|----requirements.txt   # Given (List of Flask modules for Web API)
|----main.tf            # Requested (Terraform config file)
|----docker-compose.yml # Requested
|----Dockerfile         # Requested
|----cfn-template.yml   # Requested (Optional)
```

## Expected Outcome
![Bookstore API Home Page](bookstore.jpg)
